...
data_titre <- read.csv( file = here::here("df_soro_model.csv")) %>%  mutate(age_group = factor(age_group, levels = c("<=5", "5-18", "19-59", "60-74", "75+"))) %>% select(!X) 
exp_prior <- read.csv(file = here::here("exp_prior.csv")) %>% select(!X)
...
outputs_modelA <- list(seroModel_p1)

save_info_list <- 
  list(file_name = "base_e3", model_name = "base_hier")

model_run_info <- 
  list(model = outputs_modelA, save = save_info_list)

saveRDS(model_run_info, file = here::here("base_hier.RDS"))

models <- readRDS(here::here("base_hier.RDS"))
